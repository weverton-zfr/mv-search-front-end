...
export default function Payment(){
    ...
    const plain = useLocation()
    const { state } = useLocation()
    const paymentData = state.paymentData.payment
    const amountCents = paymentData.amountCents/100
    useEffect(() => {
        const interval = setInterval(async () => {
            try {
            const response = await api.get(
                `/payments/payment-status/${paymentData.paymentId}`
            )
    ...
    return(
        <section className="flex justify-center items-center w-full h-screen">
            <div className="w-[70%] bg-black/70 rounded-md border border-green-900/50 flex  items-center gap-10 flex-col overflow-auto py-10">
                ...
                <h2 className=" text-gray-200">{paymentData.productName} por: {amountCents.toLocaleString('pt-BR', {
                    style: 'currency',
                    currency: 'BRL',
                })}
                </h2>
                <div className="w-[80%] flex flex-col gap-10 items-center">
                    <img src={paymentData.pix.qrCodeImage} alt="Codigo QR de cobranca do plano" className="size-90 bg-white text-black" />
                    <span className="p-5 bg-black/30 rounded-md w-[80%] text-gray-300">
                        <p>Chave Copia e Cola: {paymentData.pix.brCode}</p>
                    </span>
                    <span className="p-5 bg-black/30 rounded-md w-[80%] text-gray-300">
